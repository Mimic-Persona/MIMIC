...
public class GameScene extends PixelScene {
	...
	public static ManualAPI myManualAPI;
	...
	public synchronized void update() throws JSONException {
		...
		if (!Actor.processing() && Dungeon.hero.isAlive()) {
			...
			if (Dungeon.prevAction.isEmpty() && Dungeon.hero.curAction != null) {
				Dungeon.prevAction.put("action", Dungeon.hero.curAction.toString());
			}

			Dungeon.prevEnv = Status.getStatus().toString();
		...
		} else if (Dungeon.isInAgentMode) {
			AgentAPI.handle();

		} else if (Dungeon.isInMonkeyMode) {
			if (System.currentTimeMillis() - Dungeon.startTime < Dungeon.DURATION && actionCounter < MAX_ACTIONS) {	// TODO: CHANGED
				if ( Dungeon.prevTime <= 0 || System.currentTimeMillis() - Dungeon.prevTime >= Dungeon.MONKEY_INTERVAL ) {
					MonkeyAPI.handle();
					Dungeon.prevTime = System.currentTimeMillis();
				}
			} else {
				Dungeon.isInMonkeyMode = false;
				GLog.h("Monkey Mode Disabled!");
			}

		} else if (Dungeon.isInLLMMode) {
			AgentAPI.handle();
		}

		// If the hero is resting, report it at every turn
		if (Dungeon.hero.resting && Actor.upd) {
			Actor.upd = false;
			Dungeon.toReport = true;
			Dungeon.prevAction.put( "action", "Wait" );
			Dungeon.prevAction.put( "tile", "(" + Position.getHeroPositionXY()[0] + ", " + Position.getHeroPositionXY()[1] + ")");
		}
//
//		if (!Dungeon.prevAction.isEmpty() && isUpdated) {
//			GLog.h("HERE!" + isUpdated);
//			Dungeon.prevAction = new JSONObject();
//		}

		if (Dungeon.BY_ACTION && Dungeon.toReport && Dungeon.isGameServerOpen) {
			JSONObject report = new JSONObject();

			if (Dungeon.isInAgentMode) {
				try {
					// Report only when the scene updated after the hero action
...
//						Dungeon.gameServer.broadcast(report.toString());
	...
	public static void gameOver() {
		...
		if (Dungeon.IS_IN_EXP && System.currentTimeMillis() - Dungeon.startTime < Dungeon.DURATION) {
			Dungeon.hero = null;
