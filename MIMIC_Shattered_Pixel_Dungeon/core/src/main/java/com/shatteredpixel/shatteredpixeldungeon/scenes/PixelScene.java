...
import com.shatteredpixel.shatteredpixeldungeon.APIs.ManualAPI;
...
public class PixelScene extends Scene {
	...
	public static JSONObject config = jsonFileToDictionary("../config.json");
	...
	public void update() throws JSONException {
		// If it is in exp and is out of the time budget, then game over
		if (Dungeon.IS_IN_EXP && Dungeon.startTime != -1 && System.currentTimeMillis() - Dungeon.startTime >= Dungeon.DURATION) {
			...
			Dungeon.isInAgentMode = false;
			Dungeon.isInMonkeyMode = false;
			Dungeon.isInLLMMode = false;
			Dungeon.isInManualMode = false;
		...
		if (DeviceCompat.isDesktop() && fullscreenListener == null){
			KeyEvent.addKeyListener(fullscreenListener = new Signal.Listener<KeyEvent>() {
				...
				public boolean onSignal(KeyEvent keyEvent) {
					...
					if (keyEvent.pressed &&
							(action == SPDAction.GAME_SERVER || action == SPDAction.AGENT_MODE || action == SPDAction.LLM_MODE)) {
						Dungeon.isGameServerOpen = !Dungeon.isGameServerOpen;

						if (Dungeon.isGameServerOpen) {
							GLog.c("Game Server Opened!");
							Dungeon.startTime = System.currentTimeMillis();
							GameScene.gameServer = new GameServer(PORT);
							Dungeon.gameServer = GameScene.gameServer;
							Dungeon.gameServer.setConnectionLostTimeout(0);
							Dungeon.gameServer.start();

						} else {
							GLog.c("Game Server Closed!");
							try {
								Dungeon.gameServer.stop();
								AgentAPI.isWndShown = false;
							} catch (InterruptedException e) {
								throw new RuntimeException(e);
							}
						}
					...
					if (keyEvent.pressed && action == SPDAction.MANUAL_MODE){
						Dungeon.isInManualMode = !Dungeon.isInManualMode;

						if (Dungeon.isInManualMode) {
							GLog.c("Manual Mode Enabled!");
							GameScene.myManualAPI = new ManualAPI();
							Dungeon.startTime = System.currentTimeMillis();
						}
						else GLog.c("Manual Mode Disabled!");

						Dungeon.isAgentNext = Dungeon.isInManualMode;
						return true;
					}

					if (keyEvent.pressed && (action == SPDAction.AGENT_MODE ||
							action == SPDAction.LLM_MODE)) {
						Dungeon.isInAgentMode = !Dungeon.isInAgentMode;

						if (Dungeon.isInAgentMode) {
							GLog.c("MIMIC Mode Started with " + Dungeon.DURATION + " milliseconds!");
							Dungeon.startTime = System.currentTimeMillis();
						}
						else {
							GLog.c("MIMIC Mode Disabled!");
                            try {
                                Dungeon.gameServer.stop();
								AgentAPI.isWndShown = false;
                            } catch (InterruptedException e) {
						...
						Dungeon.isAgentNext = Dungeon.isInAgentMode;
						return true;
					}

					if (keyEvent.pressed && action == SPDAction.MONKEY_MODE){
						Dungeon.isInMonkeyMode = !Dungeon.isInMonkeyMode;

						if (Dungeon.isInMonkeyMode) {
							GLog.c("Monkey Mode Started with " + Dungeon.DURATION + " milliseconds!");
							Dungeon.startTime = System.currentTimeMillis();
						}
						else {
							GLog.c("Monkey Mode Disabled!");
						}

						Dungeon.isAgentNext = Dungeon.isInMonkeyMode;
						return true;
					}

					if (keyEvent.pressed && action == SPDAction.LLM_MODE){
						Dungeon.isInLLMMode = !Dungeon.isInLLMMode;

						if (Dungeon.isInLLMMode) {
							GLog.c("MIMIC Ablated Mode Started with " + Dungeon.DURATION + " milliseconds!");
						}
						else {
							GLog.c("MIMIC Ablated Mode Disabled!");
							try {
								Dungeon.gameServer.stop();
								AgentAPI.isWndShown = false;
							} catch (InterruptedException e) {
						...
						Dungeon.isAgentNext = Dungeon.isInLLMMode;
						return true;
					}

					if (keyEvent.pressed && action == SPDAction.RESTART){
//						if (GamesInProgress.selectedClass == null) return true;
