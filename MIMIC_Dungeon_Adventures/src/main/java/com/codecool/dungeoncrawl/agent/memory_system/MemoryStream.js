...
class MemoryStream {
    ...
    async init(socket) {
        ...
        this.personaDescription = await loadFile(`./src/main/java/com/codecool/dungeoncrawl/agent/context/personalities/${this.persona}.txt`, BOT_ERR_MSG);
    ...
    async inheritHistory() {
        ...
        for (let i of Object.keys(memories)) {
            ...
            await this.addMemory(memories[i].memoryType, memories[i].isSuccess,
                ...
                memories[i].task, memories[i].action, memories[i].tile, memories[i].object,
     ...
     * @param {string} object The object of the task
