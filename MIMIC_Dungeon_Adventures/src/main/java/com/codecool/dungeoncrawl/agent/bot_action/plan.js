 ...
 * @returns {Promise<{task: *, reasoning: (string|*), tile: any, action: *, object: *}|null>}
 */
async function plan(socket, memoryStream, status, personality, latestBadPlans, retrieveMethod=false, prefix="bottomUp") {
    const persaContext = fs.readFileSync(`./src/main/java/com/codecool/dungeoncrawl/agent/context/personalities/${personality}.txt`, 'utf8');
    const persaExampleContext = fs.readFileSync(`./src/main/java/com/codecool/dungeoncrawl/agent/context/personalities/${personality}_examples.txt`, 'utf8');

    let context;

    if (retrieveMethod)
        context = fs.readFileSync(`./src/main/java/com/codecool/dungeoncrawl/agent/context/${prefix}_plan_prompt_RP.txt`, 'utf8');
    else
        context = fs.readFileSync(`./src/main/java/com/codecool/dungeoncrawl/agent/context/${prefix}_plan_prompt_U.txt`, 'utf8');
