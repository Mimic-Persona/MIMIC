const { listFiles, mkdir, writeFile, loadFile, loadSkills, capitalizeFirstChar, lowercaseFirstChar} = require("../utils/file_utils");
...
class SkillManager {
     ...
     */
    constructor(socket, rootPath, importPath, collectionName, persona, isInherit = false, retrieverTopN = 5) {
    ...
    async init() {
        ...
        this.skills_import = await loadFile("./src/main/java/com/codecool/dungeoncrawl/agent/skill_library/skills_import.txt", BOT_ERR_MSG);
        ...
        mkdir(this.rootPath, `${this.persona}/compiled_code`, BOT_LOG_MSG, BOT_ERR_MSG);
    ...
    async addSkill(name, description, code, persona, isInheriting=false, relatedSkills=[]) {
        ...
        if (!isInheriting) {
            await writeFile(`${this.rootPath}/${persona}/description/${capitalizeFirstChar(name)}.txt`,
                `public class ${capitalizeFirstChar(name)} {\n` +
                `\tpublic static void ${lowercaseFirstChar(name)}() {\n` +
                `\t\t// ${description}\n` +
                "\t}\n" +
            ...
            for (let skl of relatedSkills) {
                if (skl !== name)
                    RSImports += `import ${this.importPath}.${this.persona}.code.${capitalizeFirstChar(skl)};\n`
            }

            await writeFile(`${this.rootPath}/${persona}/code/${capitalizeFirstChar(name)}.java`,
                `package ${this.importPath}.${this.persona}.code;\n\n` + this.skills_import + '\n' + RSImports + "\n" + code, BOT_LOG_MSG, BOT_ERR_MSG);
        }
